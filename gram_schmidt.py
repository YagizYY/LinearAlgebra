...
# We'll then go through the vectors one at a time and set them to be orthogonal to all the vectors that came before it. Before normalising.
...
def gsBasis4(A):
    ...
    B[:, 1] = B[:, 1] - B[:, 1] @ B[:, 0] * B[:, 0]
    # If there's anything left after that subtraction, then B[:, 1] is linearly independent of B[:, 0]
...
# %%
# Once you've done Gram-Schmidt once, doing it again should give you the same result. Test this:
U = gsBasis4(V)
gsBasis4(U)
...
A = np.array([[3, 2, 3], [2, 5, -1], [2, 4, 8], [12, 2, 1]], dtype=np.float_)
gsBasis(A)
...
# %%
B = np.array([[6, 2, 1, 7, 5], [2, 8, 5, -4, 1], [1, -6, 3, 2, 8]], dtype=np.float_)
gsBasis(B)
...
C = np.array([[1, 0, 2], [0, 1, -3], [1, 0, 2]], dtype=np.float_)
gsBasis(C)
